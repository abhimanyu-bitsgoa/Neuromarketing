...
int main()
{
    ...
    */
    ...
    int truet = 1;
    setsockopt(client,SOL_SOCKET,SO_REUSEADDR,&truet,sizeof(int));
    ...
    */
    ...
    while (server > 0)
    {
        ...
        */
        ...
        int rec_status=0;
        cout<<"I will start collection data now"<<endl;
        do {
            //vector<char> buffer(1024);
          //cout<<buffer.size()<<" curr size "<<endl;
          buffer.resize(1024);
            rec_status=recv(server, buffer.data(), buffer.size()-1,0);
            if(rec_status!=-1){
            //cout << buffer.data();
